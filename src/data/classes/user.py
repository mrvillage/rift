...
class User:
    ...
    async def create(
        ...
        user_ = cls({"user": user.id, "nation": nation.id})
        await user_.save()
    ...
    async def save(self) -> None:
        await execute_query(
            "INSERT INTO users (user, nation) VALUES ($1, $2) ON CONFLICT (user) DO UPDATE SET user = $1, nation = $2;",
            self.user_id,
            self.nation_id,
        )

    async def delete(self) -> None:
        await execute_query("UPDATE users SET nation = null WHERE user = $1;", self.user_id)
