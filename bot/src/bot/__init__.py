...
async def on_message(message: discord.Message):
    ...
    if not await bot.is_owner(message.author) and ctx.command is not None and ctx.command.name not in {"mmr", "stockpile", "mmr-check", "stockpile-check", "audit"}:  # type: ignore
        return
    try:
