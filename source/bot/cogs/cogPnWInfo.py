...
class PnWInfo(commands.Cog):
    ...
    async def alliance(self, ctx, *, search=None):
        ...
        embed = rift.get_embed_author_member(user, f"[Alliance Page](https://politicsandwar.com/alliance/id={alliance_id})", timestamp=self.bot.alliances_update, footer="Data collected at").add_field(name="Alliance ID", value=alliance_id, inline=True).add_field(name="Alliance Name", value=alliance[2], inline=True).add_field(name="Alliance Acronym", value=alliance[3], inline=True).add_field(name="Color", value=alliance[4].capitalize(), inline=True).add_field(name="Rank", value=f"#{alliance[5]}", inline=True).add_field(
