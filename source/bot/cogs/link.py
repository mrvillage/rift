...
class Link(commands.Cog):
    ...
    @commands.command(name="link", aliases=["verify"],
    ...
    async def link(self, ctx, nation, user: Union[discord.Member, discord.User] = None):
        try:
            ...
            await ctx.reply(embed=rift.get_embed_author_member(
                ctx.author,
                f"No nation found with argument `{nation}`."))
            return
        ...
        if any(user.id in tuple(i) for i in links):
            await ctx.reply(embed=rift.get_embed_author_member(
                user,
                f"{user.mention} is already linked!"))
            return
        if any(user.id in tuple(i) for i in links):
            await ctx.reply(embed=rift.get_embed_author_member(
                user,
                f"Nation `{nation.id}` is already linked!"))
            return
        try:
            message = await ctx.reply(embed=rift.get_embed_author_member(
                user,
                "Fetching Discord username..."))
