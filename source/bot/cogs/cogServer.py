...
class Server(commands.Cog):
    ...
    @commands.group(name="server", aliases=["servers", "invite", "invites"], invoke_without_command=True)
    async def server(self, ctx, *args):
        ...
        try:
            ...
            await ctx.reply(embed=rift.get_embed_author_member(ctx.author, f"I couldn't find any servers matching your arguments `{'`, `'.join(args)}`"))
            return
    ...
    async def server_add(self, ctx, *args):
        ...
        good = '\n'.join(good)
        if not invites:
            await ctx.reply(embed=rift.get_embed_author_member(ctx.author, "You didn't submit any valid invites!"))
            return
        if await Confirm(embed=rift.get_embed_author_member(ctx.author, f"Are you sure you want to submit the following servers?\n{good}")).confirm(ctx):
            for arg in args:
                await rift.submit_server(invite=arg, userid=ctx.author.id)
            await ctx.reply(embed=rift.get_embed_author_member(ctx.author, f"The following servers have been submitted:\n{good}"))
        else:
            await ctx.reply(embed=rift.get_embed_author_member(ctx.author, f"No servers have been submitted."))
    ...
    async def server_edit_name(self, ctx, server_id: int, *, name):
        if server_id not in cache.servers:
            await ctx.reply(embed=rift.get_embed_author_member(ctx.author, f"`{server_id}` is not a valid server ID."))
            return
        await rift.edit_server(server_id=server_id, name=name)
        await ctx.reply(embed=rift.get_embed_author_member(ctx.author, f"Server #{server_id}'s name has been changed to `{name}`."))
    ...
    async def server_edit_invite(self, ctx, server_id: int, *, invite):
        ...
        await ctx.reply(embed=rift.get_embed_author_member(ctx.author, f"Server #{server_id}'s invite has been changed to {invite}."))
    ...
    async def server_edit_description(self, ctx, server_id: int, *, description):
        ...
        await ctx.reply(embed=rift.get_embed_author_member(ctx.author, f"Server #{server_id}'s description has been changed to:\n`{description}`."))
    ...
    async def server_edit_categories_add(self, ctx, server_id: int, *args):
        ...
        await ctx.reply(embed=rift.get_embed_author_member(ctx.author, f"Server #{server_id}'s categories have been changed to `{', '.join(categories) if not len(categories) == 0 else None}`."))
    ...
    async def server_edit_categories_set(self, ctx, server_id: int, *args):
        ...
        await ctx.reply(embed=rift.get_embed_author_member(ctx.author, f"Server #{server_id}'s categories have been changed to `{', '.join(args)}`."))
    ...
    async def server_edit_keywords_add(self, ctx, server_id: int, *args):
        ...
        await ctx.reply(embed=rift.get_embed_author_member(ctx.author, f"Server #{server_id}'s keywords have been changed to `{', '.join(keywords) if not len(keywords) == 0 else None}`."))
    ...
    async def server_edit_keywords_set(self, ctx, server_id: int, *args):
        ...
        await ctx.reply(embed=rift.get_embed_author_member(ctx.author, f"Server #{server_id}'s keywords have been changed to `{', '.join(args)}`."))
