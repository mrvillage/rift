...
async def send_message(*, receiver=None, subject=None, content=None):
    async with aiohttp.ClientSession(timeout=30) as session:
        ...
        async with session.post(
            "https://politicsandwar.com/inbox/message", data=message_data
        ) as response:
...
async def send_message_many(
    *, email=None, password=None, key=None, receivers=None, subject=None, content=None
):
    async with aiohttp.ClientSession(timeout=30) as session:
        login_data = {"email": email, "password": password, "loginform": "Login"}
        async with session.post(
            "https://politicsandwar.com/login/", data=login_data
        ) as response:
